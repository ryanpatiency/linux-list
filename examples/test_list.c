...
int main()
{
     ...
     * first parameter is used as the counter in for loop. in other words,
     * inside the
     * loop it points to the current item's list_head.
     * second parameter is the pointer to the list. it is not manipulated by the
     * macro.
     */
    printf("traversing the list using list_for_each()\n");
    list_for_each (pos, &mylist.list) {
        /* at this point: pos->next points to the next item's 'list' variable
         * and
         * pos->prev points to the previous item's 'list' variable. Here item is
         * of type struct kool_list. But we need to access the item itself not
         * the
         * variable 'list' in the item! macro list_entry() does just that. See
         * "How
         * does this work?" below for an explanation of how this is done.
         */
        tmp = list_entry(pos, struct kool_list, list);

        /* given a pointer to struct list_head, type of data structure it is
         * part of,
         * and it's name (struct list_head's name in the data structure) it
         * returns a
    ...
    printf("\n");
    /* since this is a circularly linked list. you can traverse the list in
     * reverse order
     * as well. all you need to do is replace 'list_for_each' with
     * 'list_for_each_prev'
     * everything else remain the same!
     *
     * Also you can traverse the list using list_for_each_entry() to iterate
     * over a given
    ...
    /* now let's be good and free the kool_list items. since we will be removing
     * items
     * off the list using list_del() we need to use a safer version of the
     * list_for_each()
     * macro aptly named list_for_each_safe(). Note that you MUST use this macro
     * if the loop
